...
Консольная утилита запускается в формате "castrator.exe [-s +c +d +iso]".
...
+iso - генерация g-code. Аналогичен режиму без ключей, но на выходе два файла: .PLT и .ISO. Записываюся в указанные в конфигурации директории. Ключ может работать в сочетании с +d.
